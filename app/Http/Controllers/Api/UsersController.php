...
class UsersController extends Controller {
        ...
        try {
            ...
            if (!empty($users)) {
                $data = $users->getCollection()->transform(function ($result) use ($data, $logUser) {
                    ...
                    $tempArray[] = (!is_null($result->site_name)) ? $result->site_name : '-';
                    //$tempArray[] = ucwords($result->getRoleNames());
                    $rolename=$result->roles()->first();
                    if(!empty($rolename))
                    {
                        
                            $accessType=($rolename->name=='Replen Team') ? ($result->replen_job_access==1 ? '- All Job Access' : '- Only Own Job Access') : '';
                    ...
                    $tempArray[]=($result->roles()->pluck('name')->implode(" ")) .$accessType;
                    $tempArray[] = (!is_null($result->phone_no)) ? $result->phone_no : '-';
    ...
    public
        ...
        try {
            ...
            {
                //dd($user_model->roles()->first());
                if(!empty($user_model->roles()->first()))
               {
                     if($user_model->roles->first()->id != $request->user_role)
                    {
                        if($user_model->hasAnyRole(Role::all()))
                        {
                            $user_model->removeRole($user_model->roles->first());
                        }
               ...
               $role_name=Role::where('name','Replen Team')->where('id',$request->user_role)->first();
                if(!empty($role_name))
                {
                    $user_model->replen_job_access=isset($request->replen_job_access) ? $request->replen_job_access : 1;
                }
                else
                {
                    $user_model->replen_job_access=null;
                }
            }
            else
            {
    ...
    public
            function storeImage($product) {
        if (request()->has('profile_pic')) {
            $product->update([
                'profile_pic' => request()->profile_pic->store('users', 'public'),
            ]);
    ...
    public
            function saveAttachments(Request $request) {
        $i     = 0;
        $files = $request->file('attachments');
        if (!empty($files)) {
    ...
    function deleteAttachments(Request $request) {
        if ($request) {
            ...
            try {
                $attachments = \App\Attachment::find($request->removeId);
     ...
     */
    public
            function refreshToken(\App\Http\Requests\Api\Auth\RefreshTokenRequest $request) {
        try {
            ...
            if (isset($response->access_token) && !empty($response->access_token)) {
                return $this->sendResponse('New token generated succesfully.', 200)
                                ->header('token', $response->access_token)
                                ->header('expires_in', $response->expires_in)
                                ->header('refresh_token', $response->refresh_token);
            }
            else {
