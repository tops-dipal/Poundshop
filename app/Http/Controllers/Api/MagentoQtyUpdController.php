...
use App\MagentoQtyLog;

class MagentoQtyUpdController extends Controller
{
    public $magento;
    ...
    public function index(Request $request)
    {
		try
		{
            ...
            if($this->magento->USER_NAME!='' && $this->magento->PASSWORD!='' && $this->magento->ENDPOINT!='') 
			{
	        	...
	        	$this->CRON_TITLE = 'MAGENTO_PRODUCT_QUANTITY_UPDATE';
				...
				$operation_perform=$this->update_quantity();
				...
				{
					return $this->sendResponse('Products Qty has been updated successfully', 200);
				}
				else
				{
					return $this->sendError('Products Qty has not been updated successfully, please try again', 422);
				}
			}
			else
			{
    ...
    function update_quantity()
    {
    	...
    	$product_list=$this->magento_pro->get_product_list($this->STORE_ID);    	
        ...
        if(!empty($product_list))
        {
            ...
            {                
                ...
                {
                    $sku = $value['sku'];
                    $stockItemArray['stock_item'] = Array("qty" => $value['quantity']);
                    $postData = Array('product' => Array("extension_attributes" => $stockItemArray));
                    ...
                    if(isset($resultData) && $resultData->sku==$sku) 
                    {
                        $update_into_db[] = array(
                            "id"                 => $value['id'],
                            "is_quantity_posted" => "1",
                ...
                {
                	$magentoqtylog= new MagentoQtyLog;
                    $data=Batch::update($magentoqtylog, $update_into_db, 'id');	
                    $operation_perform=1;
                }
