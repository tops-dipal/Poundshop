...
class MagentoProReviseController extends Controller
{
    ...
    public function index(Request $request)
    {
        try
        {
            ...
            if($this->magento->USER_NAME!='' && $this->magento->PASSWORD!='' && $this->magento->ENDPOINT!='') 
            {
                ...
                $this->CRON_TITLE = 'MAGENTO_PRODUCT_REVISE_POSTING';
                ...
                $operation_perform=$this->product_post_update();                
    ...
    public function product_post_update()
    {
        $products_limit = 20;
        $operation_perform=0;
        $product_attribute_id = array();
        $product_attr_options = array();
        $product_result =$this->magento_pro_posting->getMagePostingProductList($this->STORE_ID,$products_limit);               
        ...
        if (!empty($product_result) && !empty($product_result_arr)) 
        {
            ...
            {
                $error = Array();                
                $magento_product_variation_result=MagentoProductPosting::where('parent_id',$value['magento_id'])->where('is_revised','1')->where('store_id',$this->STORE_ID)->get();
                ...
                if (!empty($magento_product_variation_result) && !empty($magento_product_variation_result_array))
                {
                    ...
                    foreach ($magento_product_variation_result as $vari_key => $vari_value)
                    {
                        ...
                        $postData = Array('product' => Array(
                            "sku"               => isset($vari_value['sku'])?$vari_value['sku']:'',
                            "name"              => isset($vari_value['product_title'])?$vari_value['product_title']:'',
                            "price"             => isset($vari_value['selling_price'])?$vari_value['selling_price']:'',
                            "status"            => 1,
                            'visibility'        => 0,
                            "type_id"           => "simple",
                            "attribute_set_id"  => 4,
                        ...
                        if ($vari_value['main_image_url'] && !strstr($vari_value['main_image_url'], $this->image_pre_url)) 
                        {
                        ...
                        {                            
                            ...
                            foreach ($multiple_image_array as $images) 
                            {
                                ...
                                {
                                    if (!strstr($image_url['image_url'], $this->image_pre_url)) 
                                    {
                        ...
                        if ($vari_value['variation_theme'] && !empty($vari_value['variation_theme_value'])) 
                        {
                            ...
                            foreach ($variation_theme as $theme_key => $theme) 
                            {
                                ...
                                {                                    
                                    ...
                                    {
                                        $product_attribute_id = $product_attr->attribute_id;
                        ...
                        $result = $this->magento->send_request("V1/products/".$vari_value['sku'], $postData, 'PUT');
                ...
                if(!empty($custom_attribute_array))
                {
                    ...
                    foreach ($custom_attribute_array as $cust_code => $cust_label)
                    {
                        if(isset($this->CUSTOM_ATTRIBUTE_DATATYPE_ARRAY[$cust_code]) && $this->CUSTOM_ATTRIBUTE_DATATYPE_ARRAY[$cust_code] == 'select')
                        {
                            if(isset($value[$this->CUSTOM_ATTRIBUTE_DATABASE_ARRAY[$cust_code]]) && !empty($value[$this->CUSTOM_ATTRIBUTE_DATABASE_ARRAY[$cust_code]]))
                            {    
                                $sel_value = '';
                                $cust_attr_d = $custom_attribute_detail[$cust_code];
                                if (!empty($cust_attr_d->options)) 
                                {
                                    foreach ($cust_attr_d->options as $key => $attr_value) 
                                    {
                                        if (!empty($attr_value->label)) 
                                        {
                                            if($value[$this->CUSTOM_ATTRIBUTE_DATABASE_ARRAY[$cust_code]] == $attr_value->label || $value[$this->CUSTOM_ATTRIBUTE_DATABASE_ARRAY[$cust_code]] == $attr_value->value)
                                            {     
                                                $sel_value = $attr_value->value;
                                            }
                                        }
                                ...
                                if(empty($sel_value))
                                {
                                    $sel_value = $this->create_attribute_option($cust_code,$value[$this->CUSTOM_ATTRIBUTE_DATABASE_ARRAY[$cust_code]]);
                                }
                        ...
                        { 
                            if($cust_code == 'category_ids')
                            {
                                $custom_attributes[] = array('attribute_code' => $cust_code,'value' => empty($value['category']) ? array('2') : explode(',', $value['category']));
                ...
                if (!empty($value['main_image_url']) && !strstr($value['main_image_url'], $this->image_pre_url)) 
                {             
                ...
                $result = $this->magento->send_request("V1/products/".$value['sku'], $config_data, 'PUT');
