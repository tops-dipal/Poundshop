...
//                }
                ...
                if($("#po_import_type").val()==1){
                    $('#po-items-container tr').each(function(row, tr){
                       TableData[row]={
                           "id":$(tr).find('td:eq(0)').attr("id"),
                            "product_id" : $(tr).find('input[type="checkbox"]').val()
                           , "supplier_sku" :$(tr).find('.supplier_sku').val()
                           , "new_barcode" :$(tr).find('.barcode').val()
                           , "bar_code" :$(tr).find('.barcode').attr("data-barcode")
                           , "qty_per_box" :$(tr).find('.qty_per_box').val()
                           , "total_box" : $(tr).find('.total_box').val()
                           , "total_quantity" : $(tr).find('.total_quantity').val()
                           , "unit_price" : $(tr).find('.unit_price').val()
                           , "total_product_cost" : $(tr).find('.total_product_cost').text()
                           , "vat" : $(tr).find('.vat').text()
                           , "standard_rate" : $("#hidden_standard_rate").val()
                           , "zero_rate" : $("#hidden_standard_rate").val()
                           , "standard_rate_value" : $(tr).find('.standard_rate').val()
                           , "zero_rate_value" : $(tr).find('.zero_rate').val()
                           , "best_before_date" : $(tr).find('.best_before_date').val()
                           , "expected_mros" : $(tr).find('.expected_mros').val()
                           , "sel_qty" : $(tr).find('.sel_qty').val()
                           , "sel_price" : $(tr).find('.sel_price').val()
                           , "mros" : 20 //$(tr).find(".mros").val()
                           , "landed_product_cost" : $(tr).find('.landed_product_cost').val()
                           , "vat_type" : $(tr).find('.vat_type').val()
                           , "net_selling_price_excluding_vat" : $(tr).find('.net_selling_price_excluding_vat').val()
                           , "total_net_selling_price" : $(tr).find('.total_net_selling_price').val()
                           , "total_net_profit" : $(tr).find('.total_net_profit').val()
                           , "total_net_margin" : $(tr).find('.total_net_margin').val()
                    ...
                    $('#po-items-container tr').each(function(row, tr){
                       TableData[row]={
                           ...
                           , "total_box" : $(tr).find('.total_box').val()
                           , "total_quantity" : $(tr).find('.total_quantity').val()
                           , "cube_per_box" : $(tr).find('.cube_per_box').val()
                           , "total_num_cubes" : $(tr).find('.total_num_cubes').val()
                           , "unit_price" : $(tr).find('.unit_price').val()
                           , "total_product_cost" : $(tr).find('.total_product_cost').text()
                           , "import_duty" : $(tr).find('.import_duty').attr("data-value")
                           , "vat" : $(tr).find('.vat').attr("data-value")
                           , "vat_type" : $(tr).find('.vat_type').val()
                           , "standard_rate" : $("#hidden_standard_rate").val()
                           , "zero_rate" : $("#hidden_standard_rate").val()
                           , "standard_rate_value" : $(tr).find('.standard_rate').val()
                           , "zero_rate_value" : $(tr).find('.zero_rate').val()
                           , "delivery_charge" : $(tr).find('.total_delivery_charge').text()
                           , "landed_product_cost" : $(tr).find('.landed_product_cost').text()
                           , "landed_price_in_pound" : $(tr).find('.landed_price_in_pound').val()
                           , "best_before_date" : $(tr).find('.best_before_date').val()
                           , "expected_mros" : $(tr).find('.expected_mros').val()
                           , "sel_qty" : $(tr).find('.sel_qty').val()
                           , "sel_price" : $(tr).find('.sel_price').val()
                           , "mros" : 20 //$(tr).find(".mros").val()
                           , "vat_in_amount" : $(tr).find('.vat_in_amount').val()
                           , "import_duty_in_cost" : $(tr).find('.import_duty_in_cost').val()
                           , "itd_vat" : $(tr).find('.totalProductCostImportDutyDeliveryCharge').val()
                           , "total_vat" : $(tr).find('.total_vat').val()
                           , "currency_exchange_rate" : $(tr).find('.currency_exchange_rate').val()
                           , "total_net_selling_price" : $(tr).find('.total_net_selling_price').val()
                           , "net_selling_price_excluding_vat" : $(tr).find('.gross_sel_price_exc_vat').val()
                           , "total_net_profit" : $(tr).find('.total_net_profit').val()
                           , "total_net_margin" : $(tr).find('.total_net_margin').val()
                           ,"po_import_type":$("#po_import_type").val()
                       }
              ...
              if(parseFloat(undefinedValue($("#hidden_min_po_amount").val())) >  parseFloat(undefinedValue($("#sub_total").text()))){
                    bootbox.alert({
                        title: "Alert",
                        message: "PO amount is less then supplier minimum po amount.",
                        size: 'small'
                    });
              }
                 $.ajax({
...
//         }
  ...
  $("#show-modal-btn").click(function(){
     ...
     $("#example > tbody").html('<td colspan="5" class="dataTables_empty py-4" valign="top">No Records Found</td>');
   ...
   $.fn.extend({
    ...
    var URL=$("#search_product_url").val();
      ...
      if(search_data == ""){
          ...
          $("#example > tbody").html('<td colspan="5" class="dataTables_empty" valign="top">No Records Found</td>');
          return false;
      }
            ...
            success: function (response) {
               $("#example_processing").hide();
                if (response.status == 1) {
                  ...
                  $("#example >tbody").html('<tr><td colspan="5" align="center"><a id="addNewBarcodeBtn" class="btn btn-blue mt-3" data-string="'+search_data+'" href="javascript:;">Add New Barcode Product</a></td></tr>');
                }
            },
            error: function (xhr, err) {
               $("#example_processing").hide();
  ...
  addBlankPO=function(barcode,isVatRateAdded){
               ...
               if($("#po_import_type").val()==1){ //UK PO
                ...
                DataHtml +='<td><div class="d-flex group-item"><span class="title color-light w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.sku+'</span><span class="desc color-blue">-</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.title+'</span><span class="desc color-light">-</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.status+'</span><span class="desc">--</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.supplier_sku+'</span><span class="desc"><input type="text" class="color-blue supplier_sku po_textbox" name="supplier_sku_'+randId+'" maxlength="15" /></span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.best_before+'</span><span class="desc color-blue"><input type="text" name="best_before_date_'+randId+'" class="best_before_date po_textbox w-150" autocomplete="off" /></span></div></td>';
                DataHtml +='<td><input data-barcode=""   maxlength="20"  type="text" name="barcode_'+randId+'" class="barcode po_textbox" value="'+barcode+'" /></td>';
                DataHtml +='<td>';
                ...
                DataHtml +='<tr>';
                DataHtml +='<td id=""><div class="d-flex"><label class="fancy-checkbox"><input name="ids[]" type="checkbox" class="child-checkbox" value="" /><span><i></i></span></label></div></td>';
                DataHtml +='<td><div class="d-flex group-item"><span class="title color-light w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.sku+'</span><span class="desc color-blue">--</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.title+'</span><span class="desc color-light">-</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.status+'</span><span class="desc">--</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.supplier_sku+'</span><span class="desc"><input type="text" class="color-blue supplier_sku po_textbox" name="supplier_sku_'+randId+'" maxlength="15" /></span></div><div class="d-flex group-item"><span class="title  w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.best_before+'</span><span class="desc"><input type="text" name="best_before_date_'+randId+'" class="best_before_date po_textbox w-150" autocomplete="off" /></span></div></td>';
                DataHtml +='<td><input data-barcode=""   maxlength="20"  type="text" name="barcode_'+randId+'" class="barcode po_textbox" value="'+barcode+'" /></td>';
                DataHtml +='<td>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.total_qty+'</span><span class="desc"><input  maxlength="9"   type="text" name="total_quantity_'+randId+'" class="total_quantity po_textbox"  /></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.qty_per_box+'</span><span class="desc"><input maxlength="8"  type="text" name="qty_per_box_'+randId+'" class="qty_per_box po_textbox" /></span></div>';
                ...
                DataHtml +='<td><div class="position-relative"><span class="pound-sign">&#163;</span><input type="text" maxlength="9" name="unit_price_'+randId+'" class="unit_price po_textbox"/></div></td>';
                DataHtml +='<td><span class="title">&#163;</span><span  name="total_product_cost_'+randId+'" class="title total_product_cost po_textbox">0.00</span></td>';                DataHtml +='<td>';
                var textRateValue=1;
                if(isVatRateAdded == true)
                {
                     textRateValue=2;
                ...
                DataHtml +='<td><div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.expected+'</span><span class="desc"><input type="text" name="expected_mros_'+randId+'" class="expected_mros po_textbox"  /></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.selling_qty+'</span><span class="desc"><input type="text" name="sel_qty_'+randId+'" class="sel_qty po_textbox" maxlenth="5" value="1" /></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.selling_price+'</span><span class="desc"><div class="position-relative"><span class="pound-sign">&#163;</span><input type="text" name="sel_price_'+randId+'" class="sel_price po_textbox" maxlength="9" /></div></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.mros+'</span><span class="desc"><input type="text" value="20" readonly="readonly"  name="mros_'+randId+'" class="mros po_textbox" maxlength="9" /></span></div>';
                DataHtml +='</td>';
  ...
  bindItemsData = function(data){
                ...
                if(data.sku == "")
                    data.sku="--"
                DataHtml +='<div class="d-flex group-item"><span class="title color-light w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.sku+'</span><span class="desc color-blue">'+data.sku+'</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.title+'</span><span class="desc color-light">'+data.title+'</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.status+'</span><span class="desc">--</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.supplier_sku+'</span><span class="desc"><input value="'+supplierSku+'" type="text" class="color-blue supplier_sku po_textbox" name="supplier_sku_'+data.id+'" maxlength="15" /></span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.best_before+'</span><span class="desc"><input type="text" name="best_before_date_'+data.id+'" class="best_before_date po_textbox w-150" autocomplete="off" /></span></div></td>';
                DataHtml +='<td><input data-barcode="'+barcode+'"   maxlength="20"  type="text" name="barcode_'+data.id+'" class="barcode po_textbox" value="'+barcode+'" />';
                DataHtml +='<td>';
                ...
                DataHtml +='<td  align="center"><input type="hidden" class="vat_type" value="'+data.vat_type+'" /><input type="hidden" class="landed_product_cost" /><input type="hidden" class="net_selling_price_excluding_vat" /><input type="hidden" class="total_net_selling_price " /><input type="hidden" class="total_net_profit" /><input type="hidden" class="total_net_margin" /> <a href="javascript:;" class="removeRow"><span class="icon-moon icon-Delete"></span></a></td>';
                DataHtml +='</tr>';
                return DataHtml;
  ...
  bindImportItemsData = function(data){
                ...
                if(data.sku == "")
                    data.sku="--"
                DataHtml +='<td id=""><div class="d-flex"><label class="fancy-checkbox"><input name="ids[]" type="checkbox" class="child-checkbox" value="'+data.id+'"/><span><i></i></span></label></div></td>';
                DataHtml +='<td><div class="d-flex group-item"><span class="title color-light w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.sku+'</span><span class="desc color-blue">'+data.sku+'</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.title+'</span><span class="desc color-light">'+data.title+'</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.status+'</span><span class="desc">--</span></div><div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.supplier_sku+'</span><span class="desc"><input type="text" value="'+supplierSku+'" class="color-blue supplier_sku po_textbox" name="supplier_sku_'+data.id+'" maxlength="15" /></span></div> <div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.best_before+'</span><span class="desc"><input type="text" name="best_before_date_'+data.id+'" class="best_before_date po_textbox w-150"  autocomplete="off"/></span></div></td>';
                DataHtml +='<td><input data-barcode="'+barcode+'"   maxlength="20"  type="text" name="barcode_'+data.id+'" class="barcode po_textbox" value="'+barcode+'" />';
                DataHtml +='<td>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.total_qty+'</span><span class="desc"><input  maxlength="9"   type="text" name="total_quantity_'+data.id+'" class="total_quantity po_textbox"  /></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-80">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.qty_per_box+'</span><span class="desc"><input maxlength="8"  type="text" name="qty_per_box_'+data.id+'" class="qty_per_box po_textbox" /></span></div>';
                ...
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.selling_qty+'</span><span class="desc"><input type="text" name="sel_qty_'+data.id+'" class="sel_qty po_textbox" maxlenth="5" value="1" /></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.selling_price+'</span><span class="desc"><div class="position-relative"><span class="pound-sign">&#163;</span><input type="text" name="sel_price_'+data.id+'" class="sel_price po_textbox" maxlength="9" value="1" /></div></span></div>';
                DataHtml +='<div class="d-flex group-item"><span class="title w-60">'+POUNDSHOP_MESSAGES.purchase_order.items.tables.mros+'</span><span class="desc"><input type="text" name="mros_'+data.id+'" readonly="readonly" class="mros po_textbox valid" maxlength="9" aria-invalid="false" value="20"></span></div>';
                DataHtml +='</td>';

                DataHtml +='<td align="center"><input type="hidden" class="vat_type" value="'+data.vat_type+'" /> <input type="hidden" class="vat_in_amount" /> <input type="hidden" class="import_duty_in_cost" /><input type="hidden" class="totalProductCostImportDutyDeliveryCharge" /><input type="hidden" class="total_vat" /><input type="hidden" class="currency_exchange_rate" /><input type="hidden" class="landed_price_in_pound" /><input type="hidden" class="total_net_selling_price" /><input type="hidden" class="gross_sel_price_exc_vat" /><input type="hidden" class="total_net_profit" /><input type="hidden" class="total_net_margin" /><a href="javascript:;" class="removeRow"><span class="icon-moon icon-Delete"></span></a></td>';
                DataHtml +='</tr>';
                return DataHtml;
  }
    
  $(document).on("keypress",".unit_price,.expected_mros,.sel_price,#total_delivery,.standard_rate,.zero_rate,.cube_per_box",function (e) {
  ...
  $(document).on("keydown",".qty_per_box,.total_box,.total_quantity,.sel_qty,#total_space", function(e) {
  ...
  bindProductData = function(data){
      ...
      for(var i = 0; i< data.length; i++){
            ...
            varDataHtml+='<td>'+data[i].sku+'</td>';
            varDataHtml+='<td>'+data[i].product_identifier+'</td>';
            varDataHtml+='<td>'+data[i].title+'</td>';
            varDataHtml+='</tr>';
