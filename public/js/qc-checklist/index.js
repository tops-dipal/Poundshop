...
{
    "user strict";
    var dataTableId = 'checklist_table';
    var poundShopDuty = function ()
    {
    ...
    c._listingView = function(){
        ...
        PoundShopApp.commonClass.table = PoundShopApp.commonClass._generateDataTable(PoundShopApp.commonClass.table,'importduty_table','api-qc-checklist',field_coloumns,order_coloumns,undefined,undefined,'Search by Code');        };
        ...
        var id = $(this).attr("id"); 
         bootbox.confirm({           
            ...
            callback: function (result) 
            {
                ...
                {
                     $.ajax({
                        url: BASE_URL + 'api-qc-checklist-remove/'+id,
                        type: "post",
                        processData: false,
                        data:{id:id},
                        headers: {
                ...
                callback: function (result) 
                {
                    if(result==true)
                    {
                        var join_selected_values = allVals.join(","); 
                         $.ajax({
                            url: BASE_URL + 'api-qc-checklist-remove-multiple',
                            type: "post",
                            processData: false,
...
window.PoundShopApp.poundShopDuty = new poundShopDuty();
...
{   
    ...
    $('#commoditycode_table tbody').on('click', 'input[type="checkbox"]', function(e)
    {
...
printQC = function(id)
{
    ...
    var popup = window.open(WEB_BASE_URL+'/qc-checklist-print-qc?id='+id,"popupWindow", "width=900, height=600, scrollbars=yes,top="+top+", left="+left);
}
